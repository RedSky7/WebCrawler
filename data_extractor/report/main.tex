...
After that the auto_ex function was called to produce a union--free regular expression.
When checking for node mismatches we mainly relied on the type of tag the node represented, the id of the node, if it didn't contain more than 3 numbers which generally described an ad node, and sequential attributes of the tag to determine if the two nodes in seperate trees are the same.
The pseudocode of the function is described bellow. 
